...
VOLUME_CONTAINER="${VOLUME_CONTAINER:-volume}"
...
WRITE_BATCH_PROBE_SIZE="${WRITE_BATCH_PROBE_SIZE:-256M}"
...
writer_pagecache_writeback_batch_ops_before="$(worker_counter "${writer_worker}" kernel_write_rdma_pagecache_writeback_batch_ops)"
writer_pagecache_writeback_batch_entries_before="$(worker_counter "${writer_worker}" kernel_write_rdma_pagecache_writeback_batch_entries)"
writer_pagecache_writeback_batch_fallbacks_before="$(worker_counter "${writer_worker}" kernel_write_rdma_pagecache_writeback_batch_fallbacks)"
...
if is_truthy "${writer_pagecache_writeback}"; then
  ...
  if [ "${RUN_WRITE_BATCH_PROBE}" = "true" ] && [ "${ASSERT_KERNEL_WRITE_PIPELINED_WRS}" = "true" ]; then
    ...
    log "kernel_write_rdma_pagecache_writeback_batch_ops on ${writer_worker}: ${writer_pagecache_writeback_batch_ops_before} -> ${writer_pagecache_writeback_batch_ops_after}"
    log "kernel_write_rdma_pagecache_writeback_batch_entries on ${writer_worker}: ${writer_pagecache_writeback_batch_entries_before} -> ${writer_pagecache_writeback_batch_entries_after}"
    log "kernel_write_rdma_pagecache_writeback_batch_fallbacks on ${writer_worker}: ${writer_pagecache_writeback_batch_fallbacks_before} -> ${writer_pagecache_writeback_batch_fallbacks_after}"
  fi
...
if [ "${RUN_METRICS}" = "true" ]; then
  ...
  if is_truthy "${writer_pagecache_writeback}"; then
    ...
    if [ "${RUN_WRITE_BATCH_PROBE}" = "true" ]; then
      ...
      assert_metric_increased "${writer_worker}/${WORKER_CONTAINER} native volume write commit batch bytes" "${writer_metrics_before}" "${writer_metrics_after}" volume_native_rdma_write_commit_batch_bytes
